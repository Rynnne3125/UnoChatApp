package coreapp.util.constants;

/**
 * Constants for file paths and separators.
 */
public class FileConstants {
	/** Path to the user data file. */
	public static final String USER_DATA_FILE_PATH = "/database/userData.txt";

	/** Path to the user statistic data file. */
	public static final String USER_STATISTIC_DATA_FILE_PATH = "/database/userStatisticData.txt";

	/** Separator for user data. */
	public static final String USER_DATA_SEPARATOR = ":";

	/** Separator for user statistic data. */
	public static final String USER_STATISTIC_DATA_SEPARATOR = ":";

	/** Path to the bot data file. */
	public static final String BOT_DATA_FILE_PATH = "/database/botData.txt";

	/** Separator for bot data. */
	public static final String BOT_DATA_SEPARATOR = ":";

	/** File path for error logs. */
	public static final String ERROR_LOGS_FILE_PATH = "logs/error_logs.txt";

	/** File path for game logs. */
	public static final String GAME_LOGS_FILE_PATH = "logs/game_logs.txt";
}
