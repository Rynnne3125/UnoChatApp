package coreapp.util.constants;

/**
 * Constants related to warning messages.
 */
public class WarningConstants {
	/** Warning message for prompting users to fill out all required fields. */
	public static final String FILL_INPUTS_WARNING = "Please fill out all required fields!";

	/** Warning message for prompting the user to enter the name of the game session. */
	public static final String FILL_SESSION_NAME_WARNING = "Please enter name of the game session!";

	/** Error message indicating that the user cannot play the current card. */
	public static final String YOU_CANNOT_PLAY_THIS_CARD = "You cannot play this card!";
}
